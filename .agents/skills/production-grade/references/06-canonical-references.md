...
| SRE / on-call / incident posture | §C — Google SRE book + the same-day incident loop (detect → smallest viable fix → broader hardening immediately after → release in the same window). |
| Looking for an *agent skill* the operator might already have authored or use | §E — Anthropic skills, Cursor skills, the org's own published skill catalogue. **Official > popular > custom.** |
...
## §B · Heuristic — official > popular > custom
...
Skipping a tier requires a one-line justification in the plan (every line has a reason). *"I built a custom one because the official skill doesn't exist"* is a complete justification; *"I built a custom one because I felt like it"* is not.
...
One line per book. The anchor `(R*)` says which rule each book reinforces. No edition numbers (evergreen). No quotes. No synopses.
...
- ***Designing Data-Intensive Applications*** — Kleppmann. The reference for schema/index/query co-design and for naming distributed shapes (Outbox, CQRS, Saga, Event-sourcing). *(R5, R6)*
- ***Domain-Driven Design*** — Evans. Aggregates, bounded contexts, ubiquitous language — the framing R12 reaches for when the model is the design. *(R12)*
- ***Site Reliability Engineering*** — Beyer, Jones, Petoff, Murphy (eds.). SLOs, error budgets, postmortems. *(R10, R14)*
- ***The Pragmatic Programmer*** — Hunt & Thomas. The general posture book. Useful as cross-checking voice. *(M1, R8)*
...
- ***How to Think Like a Mathematician*** — Houston. Concrete before abstract, definitions before theorems (types before implementation), find non-examples and extreme cases at design time, classify the problem structure, verify after executing. *(R1, R4, R8)*
- ***How to Solve It*** — Polya. The original problem-solving framework: understand → plan → execute → look back. *(R1, R10)*

### Architecture

- ***Clean Architecture*** — Robert C. Martin. Dependency inversion, hexagonal / ports-and-adapters structure, boundaries. The agent names the shape when the codebase uses DI frameworks or layered architecture. *(R12, R14)*

### Functional / declarative

- ***Functional Programming Jargon*** — Hemanth's curated terminology (also ★18K on GitHub). The Rosetta stone when porting an FP idiom across languages. *(R14)*

---

## §D · Curated curriculum

The operator maintains a decade-long curated study list. When the agent needs a canonical anchor for a pattern, algorithm, or design decision, cite the most authoritative OSS reference in the category by its `org/name` slug — exact, copy-pasteable, search-grep-able. Prefer official sources. The books in §C and the routing table in §A cover the high-frequency lookups; reach beyond them only when the specific category demands it.
...
## §E · Skill catalogues — official-source-first applied to skills

When the agent needs *a skill*, it looks here in order. **Official > popular > custom.**

1. **Anthropic official skills** — `anthropics/skills`. The platform-blessed catalogue. First place.
2. **Cursor official plugins / skills** — Cursor-shipped plugin cache and skill catalogue. Second place.
3. **The org's own published catalogues** — an org-shared agent-workspace package, per-repo `.agents/skills/`, user-level `.cursor/skills-cursor/`.
4. **Skill marketplaces / community** — popular GitHub-hosted skill packs and skill-directory sites. Third place.
5. **Custom-built in this repo** — only when 1–4 don't fit and the gap is named in the plan.
